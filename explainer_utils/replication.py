...
def explain(model, dataset, config, s, b_plot = False, device='cuda'):
    ...
    g = torch.Generator()
    g.manual_seed(s)
    train_dataset, test_dataset = random_split(dataset,[train_N,test_N],generator=g)
    g.manual_seed(s)
    _, test_original_dataset = random_split(orig_dataset,[train_N,test_N],generator=g)
